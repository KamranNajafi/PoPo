...
void main() {
  group('Health.fromLatency', () {
    test('classifies against the design thresholds', () {
      ...
      expect(
        Health.fromLatency(const Duration(milliseconds: 101)),
        Health.slow,
      );
  ...
  group('HealthTester', () {
    ...
    test('a failed retest clears a previously good ping', () async {
      ...
      final report = await tester.test([
        endpoint(
          'a.example',
          ping: const Duration(milliseconds: 30),
          health: Health.ok,
        ),
      ]);

      expect(
        report.tested.single.ping,
        isNull,
    ...
    test('a success resets the failure streak', () async {
      ...
      final report = await tester.test([
        endpoint('a.example', failureCount: 1),
      ]);
    ...
    test(
      'drops an endpoint once it fails the configured number of times',
      () async {
        final probe = FakeProbe({'a.example': null});
    ...
    test(
      'keeps a first-time failure, so one bad moment is not fatal',
      () async {
        final probe = FakeProbe({'a.example': null});
        final tester = HealthTester(
          probe: probe,
    ...
    test('auto-remove off keeps everything', () async {
      ...
      final tester = HealthTester(
        ...
        config: const HealthConfig(
          ...
          removeDeadAfterFailures: 1,
        ),
      );

      final report = await tester.test([
        endpoint('a.example', failureCount: 5),
      ]);
    ...
    test(
      'an unsupported platform records nothing rather than marking all dead',
      () async {
    ...
    test('tests the best-ranked first and honours the cap', () async {
      ...
      final tester = HealthTester(
        probe: probe,
        config: const HealthConfig(maxToTest: 2),
      );
    ...
    test('untested endpoints beyond the cap are kept, not discarded', () async {
      ...
      final tester = HealthTester(
        probe: probe,
        config: const HealthConfig(maxToTest: 1),
      );
      ...
      expect(
        report.tested.map((e) => e.host),
        containsAll(['top.example', 'kept.example']),
      );
    });
    ...
    test('respects the concurrency limit', () async {
      ...
      final tester = HealthTester(
        probe: probe,
        config: const HealthConfig(concurrency: 4),
      );
    ...
    test('reports progress as it goes', () async {
      ...
      final tester = HealthTester(
        probe: probe,
        config: const HealthConfig(concurrency: 1),
      );
